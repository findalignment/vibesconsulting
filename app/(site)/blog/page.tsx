...
export default function BlogPage() {
  return (
    <div className="pb-20">
      ...
      <div className="space-y-6">
        {posts.map(p => (
          <Link 
            key={p.slug} 
            href={`/blog/${p.slug}`}
            className="card p-8 block hover:border-walnut/50 transition-colors group"
          >
            <div className="flex items-start justify-between gap-4 mb-3">
              <h2 className="text-2xl font-semibold group-hover:text-walnut transition-colors">{p.title}</h2>
      ...
      <section className="mt-16 card p-8 bg-walnut/10">
